...
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!number || !question || !options.every(option => option.optionDescription && option.points) || !discussion) {
      // Menampilkan SweetAlert jika ada yang kosong
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
  ...
  return (
    <div className="container mx-auto p-0" style={{ maxWidth: '1440px' }}>
      <header className="bg-[#0B61AA] text-white p-4 sm:p-6 font-poppins" style={{ maxWidth: '1440px', height: '108px', marginLeft: '0' }}>
          <div className="container mx-auto flex justify-start items-center" style={{ marginLeft: '-76px' }}> {/* Menggeser ke kiri */}
...
const MembuatSoal = () => {
  ...
  const [weight, setWeight] = useState();
  const [discussion, setDiscussion] = useState('');
  const [options, setOptions] = useState([{ optionDescription: '', points: '' }]);
  const [pages, setPages] = useState([{ questions: [] }]);
  const [error, setError] = useState(null);
  const [activeTab, setActiveTab] = useState('buatTes'); 
  ...
  const handleBack = () => {
    if (testId && testCategory) {
      router.push(`/author/buatSoal?testId=${testId}&category=${testCategory}`);
    } else {
      console.error('Test ID atau kategori tidak ditemukan.');
    }
  };
          ...
          <div className="bg-[#FFFFFF] border border-black p-4 rounded-lg shadow-md w-full mb-6 " >
            ...
            <div className='mt-4 flex justify-end space-x-4 -mr-2'>
              <div className="flex justify-end space-x-2">
                ...
                <div className="flex justify-end space-x-2">
                  <button type="button" onClick={handleSubmit} className="bg-[#E8F4FF] border border-black px-4 py-2 hover:text-white font-poppins rounded-[15px]">
                    Simpan
                  </button>
                  <button
                    onClick={handleBack}
